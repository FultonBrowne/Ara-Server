...
class Weather {
    ...
    fun main(params:ParseUrl.ApiParams):Feed{
        ...
        println(finalData)
        val json = JsonParser().parse(finalData)
        if (time == null || time == 0L) return parse(json.asJsonObject.getAsJsonObject("currently"))
        else return parse(json.asJsonObject.getAsJsonObject("daily").getAsJsonArray("data")[time!!.toInt()].asJsonObject)


    }
    private fun parse(dataSet: JsonObject):Feed{
        println("test 3")
        val temp:String?
        temp = try {
            dataSet.asJsonObject.get("temperature").asInt.toString()
        ...
        val foreCast = dataSet.asJsonObject.get("summary").asString
        val title = "$temp and $foreCast"
        return Feed("list", null, title, arrayListOf(FeedModel(title, "Weather by darksky")))
    }
}
